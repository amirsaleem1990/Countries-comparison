...
call_log_func_default = pd.DataFrame(columns=['number', ' Type', ' DateTime', ' Duration'])
...
def parsing_error(folder_name, file_name):
	try:
		with open("/home/amir/github/LFD-projects/ETL-cash-egypt/22-oct-2019/logs.pkl", "rb") as file:
	...
	if folder_name in logs:
		logs[folder_name].append((file_name, now, "Parsing error"))
	else:
		logs[folder_name] = [(file_name, now, "Parsing error")]

	if logs != existing_logs:
		with open("/home/amir/github/LFD-projects/ETL-cash-egypt/22-oct-2019/logs.pkl", "wb") as file:
...
def file_not_found(folder_name, file_name):
	...
	if folder_name in logs:
		logs[folder_name].append((file_name, now, "File not found"))
	else:
		logs[folder_name] = [(file_name, now, "File not found")]
...
def check_empty(folder_name, file_name):
	...
	if folder_name in logs:
		logs[folder_name].append((file_name, now, "Empty file"))
	else:
		logs[folder_name] = [(file_name, now, "Empty file")]
