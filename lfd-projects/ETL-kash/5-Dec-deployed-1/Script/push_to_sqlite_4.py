...
def initialize_ETL_DB_file():
		...
		conn = sqlite3.connect('/home/amir/github/LFD-projects/ETL-kash/5-Dec-deployed-1/db/ETL.db') 
		...
		df_call_log = pd.DataFrame(columns = ['ID','number','Type', 'date', 'Duration','Date & time'])
...
if not "ETL.db" in os.listdir("/home/amir/github/LFD-projects/ETL-kash/5-Dec-deployed-1/db"):
...
def only_new_records(df, table_name, ID, Time):
	if len(df) > 0:
		if "Date & time" in df.columns:
			if table_name != "location": # location me har dafa ki update honi chahye, regardless of time
...
def Push_to_sqlite(folder_name, df_sms_log, 
	...
	Time = str(datetime.fromtimestamp(math.floor(int(ab[1])/1000)))
...
def Push_to_sqlite_Master_table(folder_name, S_phone_battery_level, S_ip_address, S_device_info, S_storage_size):
	conn = sqlite3.connect('/home/amir/github/LFD-projects/ETL-kash/5-Dec-deployed-1/db/Master.db')
	
	ID = folder_name.replace("phoneRawDataFile-", "").replace(".zip", "").split("-")[0]
	...
	if len(df_master) > 0:
		...
		df_master = df_master[["ID"] + [i for i in df_master.columns if i != "ID"]]

		try:
			df_SQL_exist = pd.read_sql_query("SELECT * FROM Master" , conn)

			only_new = df_master[~df_master.isin(df_SQL_exist.to_dict('l')).all(1)]
			if len(only_new) > 0:
				only_new["ID"] = [ID]
				only_new = only_new[["ID"] + [i.strip() for i in only_new.columns if i != "ID"]]
				only_new.to_sql('Master', conn, index=False, if_exists='append')
		except:
